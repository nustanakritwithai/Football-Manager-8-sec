...
## P3.1: โหมดอาเขต — เลือกทีม (Premier Arcade)

เปิดเกมจะเจอหน้าจอ **เลือกทีม** ก่อนเริ่มแมตช์ (กดปุ่ม **🏟 เลือกทีม / แมตช์ใหม่** เพื่อกลับมาเลือกใหม่ได้):

- ทีมสไตล์พรีเมียร์ลีกอังกฤษ **20 ทีม** — ใช้ชื่อสมมติอิงเมือง/สี (เช่น Manchester Red,
  Merseyside Red, North London Reds) **ไม่ใช้เครื่องหมายการค้าจริง** เพราะ deploy สาธารณะ
- แต่ละทีมมีเรตติ้ง **ดาว 1-5** → สเกลค่าพลังนักเตะจริง (เลือกทีมแกร่ง = ง่าย, ทีมรอง = ท้าทาย)
  พร้อม formation + สไตล์การเล่นประจำทีม
- เลือกทีมเรา + คู่แข่ง (หรือกดสุ่มคู่แข่ง) → กด **Kick Off!** เริ่มแมตช์ใหม่ทันที
- **ดาวเด่นมีชื่อ + ค่าพลังตามตัวจริง:** ทีมใหญ่มีนักเตะดาวเด่น (ชื่อ homage สะกดเพี้ยน
  เลี่ยงสิทธิชื่อจริง เช่น E. Halund, M. Sallah, B. Saca) ค่าพลังปั้นตามโปรไฟล์จริง
  (กองหน้าตัวเก่ง shooting/speed สูง, เพลย์เมกเกอร์ passing/vision สูง) — แก้เป็นชื่อจริง
  หรือเพิ่มดาวได้ที่ `src/teams-data.js` (STAR_ROSTERS) ที่เดียว

## P3.0: หน้าปรับแผน (Tactics) — แรงบันดาลใจจาก Football Manager

กดปุ่ม **⚙ ปรับแผน / Tactics** เพื่อเปิด modal เต็มจอ (ปรับได้เฉพาะตอนไม่ได้จำลอง):

- **ฝั่งซ้าย — สนามปรับตำแหน่ง:** ลากนักเตะ 11 ตัวได้อิสระ ตำแหน่งที่วางจะ **fix เป็น
  "ตำแหน่งหลัก" (baseX/baseY)** ที่นักเตะใช้วิ่งอ้างอิงในแมตช์ (steering ดึงเข้าหา base นี้)
  เลือก formation preset เป็นจุดเริ่ม แล้วปรับเองต่อได้ · ก่อนเขี่ยเริ่มเกมจะเห็นตัวขยับทันที,
  ระหว่างแมตช์นักเตะจะค่อย ๆ วิ่งเข้าหาตำแหน่งใหม่เอง
- **ฝั่งขวา — รายชื่อ (Squad):** ตัวจริง 11 + ตัวสำรอง 7 พร้อมค่าพลังหลัก (PAC/PAS/SHO/DEF)
  และ stamina · เปลี่ยนตัวด้วยการคลิกตัวจริง → คลิกตัวสำรอง (ตัวที่ลงรับตำแหน่งหลัก/บอล
  ของตัวที่ออก, stamina สด) จำกัด 5 ครั้งต่อแมตช์ ตัวสำรองไม่ถูกจำลองจนกว่าจะถูกเปลี่ยนลง
...
- **🎯 ซ้อมฟรีคิก** — จัดฟรีคิกระยะยิงพร้อมกำแพง 4 คน + ตัวรอเก็บตก
- **ลูกตั้งเตะ (P2.9)** — คนเตะลูกตั้งเตะ "ไม่เลี้ยงเอง" อีกต่อไป: เตะมุม =
  เปิดโด่งเข้ากรอบเล็งตัวที่ว่างสุด/อยู่โซนอันตราย ลุ้นปะทะโหม่ง (วัดกันด้วย aerial
  duel เดิม) · ฟรีคิก = ยิงตรงเมื่อได้ระยะ (<28m มุมเปิด) ไม่งั้นเปิดเข้ากรอบหรือ
  จ่ายสั้น
